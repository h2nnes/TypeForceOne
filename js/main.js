...
let textEditMode = false; // Toggles HTML textarea overlay editing
let currentText = (overlayTextarea && (overlayTextarea.value || overlayTextarea.placeholder)) || "";
...
function syncOverlayTextarea() {
    ...
    const rect = textField.bounds;
    const canvasRect = canvasEl.getBoundingClientRect();
    // Map Paper coordinates to canvas pixels (assumes 1:1 CSS pixels to view coordinates)
    overlayTextarea.style.left = `${canvasRect.left + rect.left}px`;
    overlayTextarea.style.top = `${canvasRect.top + rect.top}px`;
    overlayTextarea.style.width = `${rect.width}px`;
    overlayTextarea.style.height = `${rect.height}px`;
    overlayTextarea.style.fontSize = `${fontSize}px`;
}
...
function enterTextEditMode() {
    ...
    mouseRadiusCircle.visible = false;
    mouseRadiusSquare.visible = false;
    ...
    if (overlayTextarea) {
        overlayTextarea.value = currentText || "";
        overlayTextarea.style.display = 'block';
    }
    syncOverlayTextarea();
    overlayTextarea && overlayTextarea.focus({ preventScroll: true });
}
...
function exitTextEditMode() {
    ...
    if (overlayTextarea) {
        currentText = overlayTextarea.value || currentText;
...
paper.view.onMouseDown = function(event) {
    ...
    if (shapeMode === 'square' && (!hit || hit.item !== handle)) {
        if (mouseRadiusSquare && typeof mouseRadiusSquare.cycleForceDirection === 'function') {
...
document.addEventListener('mousedown', (e) => {
    if (textEditMode && el.overlayTextarea && !el.overlayTextarea.contains(e.target)) {
        exitTextEditMode();
    }
});
...
document.addEventListener('keydown', (e) => {
    if (e.key === 't' || e.key === 'T') {
        const targetIsOverlay = el.overlayTextarea && document.activeElement === el.overlayTextarea;
        ...
        if (!textEditMode) {
            ...
            e.stopPropagation();
            enterTextEditMode();
        } else if (textEditMode && !targetIsOverlay) {
